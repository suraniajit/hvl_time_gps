...
<div class="card">
    <div class="card-content">
        <h5 class="title-color"><span>Update Country</span></h5>
        <form  action="{{route('hrms.country.update',['id' => $CountryMaster[0]->id])}}" method="post" id="formValidate">
            
            <input type="hidden" id="country_id" name="country_id" value="<?php echo $CountryMaster[0]->id ?>">

            @csrf
            <div class="row">
                <div class="input-field col s6">
                    <label for="">Country Name: </label>
                    <input type="text" name="country_name" id="country_name" value="{{$CountryMaster[0]->country_name}}"  data-error=".errorTxt1" autocomplete="off" autofocus="off">
                ...
                <div class="input-field col s6">
                    <select id="is_active" name="is_active" data-error=".errorTxt2" >
                        ...
                        <option value = "0" {{ $CountryMaster[0]->is_active=='0' ? ' selected="" ' : '' }} > Active  </option>
                        <option value = "1" {{ $CountryMaster[0]->is_active=='0' ? 'Active' : ' selected="" ' }}>Inactive </option>
                    </select>
...
<script src="{{asset('js/hrms/country/edit.js')}}"></script>
