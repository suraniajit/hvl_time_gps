$(document).ready(function() {
    ...
    $('#Assignments_datatable').DataTable({
            "dom": '<"top"f>irt<"bottom"lp><"clear">',
            "processing": true,
            "serverSide": false,
            "ajax": route+"recruitment/assignments/getdata",
            "columns": [
                {"data": "id"},
...
$(document).on('click', '.delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: route+"recruitment/assignments/removedata",
                mehtod: "get",
                data: {id: id},
                success: function(data)
                {
                    $('#Assignments_datatable').DataTable().ajax.reload();
...
$(document).on('click', '#bulk_delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $('#AssignmentsCheckedbox:checked').each(function() {
                id.push($(this).val());
            });
            if (id.length > 0)
            {
                $.ajax({
                    url: route+"recruitment/assignments/massremove",
                    method: "get",
                    data: {id: id},
                    success: function(data)
                    {
