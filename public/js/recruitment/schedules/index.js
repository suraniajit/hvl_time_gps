$(document).ready(function() {
    ...
    $('#Schedules_datatable').DataTable({
            ...
            "processing": true,
            "serverSide": false,
            "ajax": route+"/recruitment/schedules/getdata",
            "columns": [
                {"data": "id"},
                {"data": "schedule_name"},
                // {"data":  "is_active"},
...
$(document).on('click', '.delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: route+"/recruitment/schedules/removedata",
                mehtod: "get",
                data: {id: id},
                success: function(data)
                {
                    $('#Schedules_datatable').DataTable().ajax.reload();
...
$(document).on('click', '#bulk_delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $('#SchedulesCheckedbox:checked').each(function() {
                id.push($(this).val());
            });
            if (id.length > 0)
            {
                $.ajax({
                    url: route+"/recruitment/schedules/massremove",
                    method: "get",
                    data: {id: id},
                    success: function(data)
                    {
