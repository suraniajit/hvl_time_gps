$(document).ready(function () {
    ...
    $('#Status_datatable').DataTable({
        ...
        "processing": true,
        "serverSide": false,
        "ajax": "/recruitment/status/getdata",
        "columns": [
            {"data": "id"},
            {"data": "status_name"},
            {"data": "is_active"},
...
$(document).on('click', '.delete', function () {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: "/recruitment/status/removedata",
                mehtod: "get",
                data: {id: id},
                success: function (data)
                {
                    $('#Status_datatable').DataTable().ajax.reload();
...
$(document).on('click', '#bulk_delete', function () {
    ...
    swal({
        ...
        if (willDelete) {
            $('#StatusCheckedbox:checked').each(function () {
                id.push($(this).val());
            });
            if (id.length > 0)
            {
                $.ajax({
                    url: "/recruitment/status/massremove",
                    method: "get",
                    data: {id: id},
                    success: function (data)
                    {
