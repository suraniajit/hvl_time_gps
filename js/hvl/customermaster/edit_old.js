...
// var form_rules = [];
// form_rules = ['employee_id[]']
// JSON.stringify(form_rules);
...
$('#formValidate').submit(function(e){
    var form_rules= {
        ...
        var collected_form_rules ={};
    var collected_form_messages ={};
    var myArray = ["billing_pincode","shipping_pincode","contact_person","contact_person_phone","billing_mail","sales_person"];
    $.each(form_rules.rules, function(key, value) {
        if (($('#is_active').val() != 1) && ($.inArray(key, myArray) != -1)) {
            collected_form_rules[key] = value;
        }
    });
    $.each(form_rules.messages, function(key, value) {
        if (($('#is_active').val() != 1) && ($.inArray(key, myArray) != -1)) {
            collected_form_messages[key] = value;
        }
    });
    console.log(collected_form_rules);
    console.log(collected_form_messages);
    $("#formValidate").validate({
        rules: collected_form_rules,
        messages:collected_form_messages,
        errorElement: 'div',
        errorPlacement: function (error, element) {
            var placement = $(element).data('error');
            if (placement) {
